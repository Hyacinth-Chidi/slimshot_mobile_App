...
internal class TimelinePlaybackEngine(
    ...
    /**
     * The volume slider's live value per clip, applied in [applyAudio] in place
     * of the clip's keyframed gain until the next `setTimeline` clears it —
     * the same shape as [transformOverrides]. `applyVolume` change-guards, so a
     * slider held still costs nothing per tick.
     */
    private val volumeOverrides = HashMap<String, Double>()

    fun setClipVolume(clipId: String, volume: Double) {
        volumeOverrides[clipId] = volume.coerceIn(0.0, 1.0)
        applyAudio(timelinePositionSeconds())
    }

    fun clearClipVolume(clipId: String) {
        if (volumeOverrides.remove(clipId) != null) applyAudio(timelinePositionSeconds())
    }

    /** The clip's gain now: the live slider value if one is held, else its keyframed volume. */
    private fun clipGain(clip: NativeTimelineClip, position: Double): Double =
        volumeOverrides[clip.id] ?: clip.volumeAt(clip.clipProgressAt(position))
    ...
    private fun seekLaneTo(lane: Lane, timelineSeconds: Double) {
        ...
        val inClipSourceSeconds = clip.sourceOffsetAt(timelineSeconds)
    ...
    fun timelinePositionSeconds(): Double {
        ...
        val inClipSeconds = clip.timelineOffsetForSource(player.currentPosition / 1000.0)
    ...
    /**
     * The overlays the preview draws. The engine only *knows about* them — it
     * supplies the list and the clock, and decides when a redraw is owed. The
     * renderer builds the draws, on the GL thread, inside its own draw: the
     * builder uploads textures and owns decoders, neither of which this
     * thread may touch.
     */
    ...
    /**
     * Tells the renderer where the overlays' clock is, and whether the picture
     * can differ because of it.
     *
     * [OverlayClock.needsRedraw] is what keeps a sticker sitting still off the
     * GPU: this runs ~60 times a second and most of those ticks cannot change
     * an overlay. The clock itself is always passed on, so a draw requested
    ...
    private fun keepIncomingInStep(
        ...
        val expectedSeconds = incoming.sourceOffsetAt(position)
    ...
    private fun applyAudio(position: Double) {
            ...
            for (lane in lanes) {
                ...
                lane.applyVolume(
                    if (lane.index == masterLane && clip != null) {
                        ...
                        (volume * clipGain(clip, position)).toFloat()
                    } else {
                        0f
                    },
                )
        ...
        val outGain = clipGain(outgoing, position)
        val inGain = clipGain(incoming, position)
